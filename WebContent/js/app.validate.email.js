app = $.extend({}, app, {
		init: function() {
			app.ajaxInit();
			app.validateEmailChange();
		},
		validateEmailChange: function() {
			app.ajax({
				servlet: "clientValidateEmail",
		...
		modifyEmail: function() {
			...
			app.ajax({
				servlet: "clientEmailChange",
				email: $("#email1").val(),
				clientId: app.clientId
			}, function(reply) {
				if(reply.object === true) {
					//alert("Verification message was sent.");
					$("#modalVerificationMessage").modal();
					$("#verificationReload").click(function() {
