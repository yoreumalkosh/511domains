...
public class domains {
	public ajaxResult addDomain(domain _domain, client _client) {
		...
		try {
			...
			String sql = "";
			sql += "INSERT INTO servermail.virtual_domains\n";
			sql += "(name, isActive, userId)\n";
			sql += "VALUES(?, ?, ?);\n";
			...
			int rowsNumber = _serverDb.preparedUpdate(preparedStatement);
	...
	public ajaxResult deleteDomain(domain _domain) {
		...
		try {
			...
			String sql = "";
			sql += "SELECT id FROM servermail.virtual_domains\n";
			sql += "WHERE name = ?;\n";

			PreparedStatement preparedStatement = _serverDb.createPreparedStatement(sql);
			preparedStatement.setString(1, _domain.getDomainName());

			ResultSet resultSet = _serverDb.preparedQuery(preparedStatement);
			if (resultSet.next()) {
				domain_id = resultSet.getLong("id");
			}
		...
		*/
		...
		try {
			...
			sql += "WHERE domain_name = ?;\n";

			PreparedStatement preparedStatement = _serverDb.createPreparedStatement(sql);
			preparedStatement.setString(1, _domain.getDomainName());

			_serverDb.preparedUpdate(preparedStatement);
		...
		try {
			...
			String sql = "";
			sql += "DELETE FROM servermail.virtual_domains\n";
			sql += "WHERE name = ?;\n";
	...
	public domain getObjectDomainId(domain _domain, client _client) {
		domain _dbDomain = new domain();
