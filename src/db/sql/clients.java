...
public class clients {
	public ajaxResult addClient(client _client) {
		...
		try {
			...
			sql += "(fullName, email, gender, password, organization, `role`, city, country, isValidated, isActive1, isActive2, isActive3, dateTimeCreated, dateTimeModified)\n";
			sql += "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);\n";
			...
			preparedStatement.setString(4, stuff.main.encryptPassword(_client.getPassword()));
			preparedStatement.setString(5, _client.getOrganization());
			preparedStatement.setString(6, _client.getRole());
			preparedStatement.setString(7, _client.getCity());
			preparedStatement.setString(8, _client.getCountry());
			preparedStatement.setBoolean(9, false);
			preparedStatement.setBoolean(10, true);
			preparedStatement.setBoolean(11, true);
			preparedStatement.setBoolean(12, true);
			preparedStatement.setTimestamp(13, now);
			preparedStatement.setTimestamp(14, now);
	...
	public ajaxResult validatePasswordReset(String passwordResetCode, client _client) {
		...
		try {
			...
			String sql3 = "";
			sql3 += "UPDATE haa.clients\n";
			sql3 += "SET `password`=?, dateTimeModified=?\n";
			sql3 += "WHERE clientId=?;\n";
			
			PreparedStatement preparedStatement3 = _db.createPreparedStatement(sql3);
			preparedStatement3.setString(1, stuff.main.encryptPassword(_client.getPassword()));
			preparedStatement3.setTimestamp(2, new Timestamp(utilNow.getTime()));
			preparedStatement3.setLong(3, clientId);
	...
	public ajaxResult modifyPassword(client _client) {
		...
		try {
			...
			String sql = "";
			sql += "UPDATE haa.clients\n";
			sql += "SET password=?, dateTimeModified=?\n";
			sql += "WHERE clientId=?;\n";
			Timestamp now = new Timestamp(new java.util.Date().getTime());

			PreparedStatement preparedStatement = _db.createPreparedStatement(sql);
			preparedStatement.setString(1, stuff.main.encryptPassword(_client.getPassword()));
			preparedStatement.setTimestamp(2, now);
			preparedStatement.setLong(3, _client.getClientId());
	...
	public boolean checkPassword(client _client) {
		...
		try {
			...
			String sql = "";
			sql += "SELECT email, password FROM haa.clients WHERE email = ? AND password = ?;\n";

			PreparedStatement preparedStatement = _db.createPreparedStatement(sql);
			preparedStatement.setString(1, _client.getEmail());
			preparedStatement.setString(2, stuff.main.encryptPassword(_client.getPassword()));
	...
	public ajaxResult searchClientJSONObjectByEmail(client _client) {
		...
		try {
			...
			String sql = "";
			sql += "SELECT clientId, fullName, email, gender, password, organization, `role`, city, country, isValidated, isActive1, isActive2, isActive3, dateTimeCreated, dateTimeModified\n";
