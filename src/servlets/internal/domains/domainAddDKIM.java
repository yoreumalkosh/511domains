 ...
 */
@WebServlet(main.internalServletsPrefix + "/domainAddDKIM")
public class domainAddDKIM extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public domainAddDKIM() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (_dbDomain.getDomainName() != null) {
				validInsert = (boolean)_dkims.insertDKIM(_dkim).retrieve().get("object");
				_ajaxResult.insert(true, jsonPair, validInsert);	
			} else {
				_ajaxResult.insert(true, jsonPair, false);
			}
			
			if (validInsert) {
				stuff.dkim.generate(domainName, selector);
				stuff.dkim.write();
				stuff.dkim.restart();
			}
