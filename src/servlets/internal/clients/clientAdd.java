 ...
 */
@WebServlet(main.internalServletsPrefix + "/clientAdd")
public class clientAdd extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			_client.setActive1(true);
			_client.setActive2(true);
			_client.setActive3(true);
			_client.setCity((String)jsonObject.get("city"));
			_client.setCountry((String)jsonObject.get("country"));
			_client.setDateTimeCreated(now);
			_client.setDateTimeModified(now);
			_client.setEmail((String)jsonObject.get("email"));
			_client.setFullName((String)jsonObject.get("fullName"));
			_client.setGender((String)jsonObject.get("gender"));
			_client.setOrganization((String)jsonObject.get("organization"));
			_client.setPassword((String)jsonObject.get("password"));
			_client.setRole((String)jsonObject.get("role"));
			_client.setValidated(false);
			boolean booleanResult = (boolean)_clients.addClient(_client).retrieve().get("object");
			
			if (booleanResult) {
				String emailValidationCode = stuff.main.generateEmailValidationCode();
				client _newClient = _clients.searchClientBeanByEmail(_client);
				_clients.createEmailValidationCode(emailValidationCode, _newClient);
				stuff.email.sendValidationEmail((String) jsonObject.get("email"), emailValidationCode);
			}
